        ...
        handler: function (request, reply) {
            ...
            switch (searchBy) {
                ...
                case 'CompanyName':
                    ...
                    break;
                case 'Date':
                    Order.populate({ createdOn: date }, { sort: { createdOn: -1 }, page: request.query.pageNo, limit: request.query.perPage }, function (err, orders) {
                        if (err) {
                            return reply(Boom.badRequest(err));
                        }
                        if (orders.total === 0) {
                    ...
                    break;
                case 'Date Range':
                    Order.populate({ createdOn: { $gte: dateFrom, $lte: dateTo } }, { sort: { createdOn: -1 }, page: request.query.pageNo, limit: request.query.perPage }, function (err, orders) {
                        if (err) {
                            return reply(Boom.badRequest(err));
                        }
                        if (orders.total === 0) {
        ...
        handler: function (request, reply) {
            ...
            switch (searchBy) {
                ...
                case 'Not Paid':
                    ...
                    break;
                case 'Date':
                    Order.populate({ userID: request.query.recruiterUserID, createdOn: date }, { page: request.query.pageNo, limit: request.query.perPage }, function (err, orders) {
                        if (err) {
                            return reply(Boom.badRequest(err));
                        }
                        if (orders.total === 0) {
                    ...
                    break;
                case 'Date Range':
                    Order.populate({ userID: request.query.recruiterUserID, createdOn: { $gte: dateFrom, $lte: dateTo } }, { page: request.query.pageNo, limit: request.query.perPage }, function (err, orders) {
                        if (err) {
                            return reply(Boom.badRequest(err));
                        }
                        if (orders.total === 0) {
...
exports.paymentNotify = {
    ...
    handler: function (request, reply) {
        /*
        notifier.emit("paid", { orderID: 1 });
        console.log('Paid Notify Emitted');
        */
        ...
        //search for order.
            async.waterfall([function (cb) {
                Order.findOne({ '_id': request.payload.m_payment_id }, function (err, order) {
                    ...
                    if (!order) {
                        reply(Boom.notFound("No orders found"));
                    }
                    if (order) {
                        //verify merchant_id
                        if (request.payload.merchant_id !== merchant_id) {
                            reply(Boom.badRequest('Invalid Merchant ID'));
                            //throw new Error('Invalid Merchant ID');
                        }

                        //verify if order has not been processed already
                        if (request.payload.pf_payment_id === order.pf_payment_id) {
                            reply(Boom.badRequest('Order has been processed already'));
                            //throw new Error('Order has been processed already');               
                        }
                        //verify amount
                        if (request.payload.amount_gross !== order.total) {
                            reply(Boom.badRequest('Amount Mismatch'));
                            //throw new Error('Amount Mismatch');
                        }
                         /*
                        //verify IP Address
                        if (!IPaddressValidate.validateIPaddress(request)) {
                            reply(Boom.badRequest('Invalid IP Address'));
                            //throw new Error('Invalid IP Address');                                             
                        }
                        //
                       
                        paymentDataEncoded = PaymentDataEncoded(request.payload);
                        //validate posted data Signature
                        if (!OrderValidateSignature(paymentDataEncoded, request.payload)) {
...
exports.deleteOrder = {
    ...
    handler: function (request, reply) {
        Order.findOneAndRemove({ '_id': request.query.orderID }, function (err, recruiter) {
            if (err) {
                return reply(Boom.badRequest(err));
            }
